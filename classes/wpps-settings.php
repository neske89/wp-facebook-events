...
if ( ! class_exists( 'WPPS_Settings' ) ) {
	...
	class WPPS_Settings extends WPPS_Module {
		...
		public function register_settings() {
			...
			add_settings_field(
				'wpps_field-facebook-token',
				'Facebook token',
				array( $this, 'markup_fields' ),
				'wpps_settings',
				'wpps_section-basic',
				array( 'label_for' => 'wpps_field-facebook-token' )
			);
		...
		public function markup_fields( $field ) {
			switch ( $field['label_for'] ) {
				case 'wpps_field-facebook-token':
		...
		public function validate_settings( $new_settings ) {
			...
			if ( empty($new_settings['basic']['field-facebook-token'])) {
				//add_notice( 'Example 1 must equal "valid data"', 'error' );
				$new_settings['basic']['field-facebook-token'] = self::$default_settings['basic']['field-facebook-token'];
			}
